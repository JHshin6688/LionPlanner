  ...
  return (
    <div className="flex h-full flex-1 flex-col overflow-hidden">
      <div className="flex border-b border-slate-200">
        ...
        {DAY_CODES.map((day) => (
          <div key={day} className="flex-1 border-l border-slate-100 py-2 text-center text-sm font-semibold text-slate-700">
            {day}
          </div>
        ))}
      </div>
        ...
        <div className="w-14 shrink-0" style={{ height: CALENDAR_HEIGHT_PX }}>
          {HOURS.map((hour) => (
            <div key={hour} style={{ height: HOUR_HEIGHT_PX }} className="relative">
              <span className="absolute -top-2 right-2 text-[11px] text-slate-400">{formatHourLabel(hour)}</span>
            </div>
          ))}
        </div>
            ...
            >
              ...
              {placedForDay.map(({ course, session }, i) => (
                ...
                >
                  ...
                  <p className="line-clamp-2 text-[10px] leading-tight">{course.course_title}</p>
                </button>
              ))}
            </div>
          )
